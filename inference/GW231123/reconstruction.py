...
outdir = "GW231123_transdimensional"
label = "GW231123"
...
N = 4  # Max number of sine-Gaussian glitch components per IFO
...
# ifo_list[0] = H1, ifo_list[1] = L1 — order matches the detectors loop above.
glitch_generators = {
...
for i in range(N):
    for prefix in ('H1', 'L1'):
        priors[f'{prefix}_dt{i}'] = bilby.core.prior.Uniform(
            0.0, duration, name=f'{prefix}_dt{i}')
        priors[f'{prefix}_f{i}'] = bilby.core.prior.Uniform(
            minimum_frequency, 200, name=f'{prefix}_f{i}')
...
priors['chirp_mass'] = bilby.core.prior.Uniform(
    minimum=60, maximum=150, name='chirp_mass', latex_label='$\\mathcal{M}$', unit='$M_\\odot$')
priors['mass_ratio'] = bilby.core.prior.Uniform(
    minimum=0.25, maximum=1.0, name='mass_ratio', latex_label='$q$')
priors['luminosity_distance'] = bilby.core.prior.PowerLaw(
    alpha=2, minimum=100, maximum=5000,
